 ...
 * - GET /v1/core/logs/{category}/{date} - Get logs of category and specific date with filtering
 */
...
public:
    METHOD_LIST_BEGIN
        ...
        ADD_METHOD_TO(LogHandler::getLogsByCategoryAndDate, "/v1/core/logs/{category}/{date}", Get);
        ...
        ADD_METHOD_TO(LogHandler::handleOptions, "/v1/core/logs/{category}/{date}", Options);
    METHOD_LIST_END
     ...
     * @param callback Response callback
     */
    void getLogsByCategory(const HttpRequestPtr &req,
                          std::function<void(const HttpResponsePtr &)> &&callback);
     ...
     * @param callback Response callback
     */
    void getLogsByCategoryAndDate(const HttpRequestPtr &req,
                                  std::function<void(const HttpResponsePtr &)> &&callback);
...
private:
     ...
     */
    std::vector<std::string> getTailLines(const std::string& file_path, int tail_count) const;
     ...
     */
    std::vector<Json::Value> readLogFile(const std::string& file_path, int tail_count = 0) const;
     ...
     */
    bool parseCategory(const std::string& category_str, LogManager::Category& category) const;
