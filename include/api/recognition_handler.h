...
public:
   ...
   */
  static void populateStorageFromDatabase(
      const std::map<std::string, std::vector<float>> &db_map);

  METHOD_LIST_BEGIN
  ADD_METHOD_TO(RecognitionHandler::recognizeFaces, "/v1/recognition/recognize",
                Post);
  ADD_METHOD_TO(RecognitionHandler::registerFaceSubject,
  ...
  ADD_METHOD_TO(RecognitionHandler::handleOptionsFaces, "/v1/recognition/faces",
                Options);
  ADD_METHOD_TO(RecognitionHandler::handleOptionsDeleteFaces,
                "/v1/recognition/faces/delete", Options);
  ADD_METHOD_TO(RecognitionHandler::handleOptionsDeleteAll,
                "/v1/recognition/faces/all", Options);
  ADD_METHOD_TO(RecognitionHandler::handleOptionsSubjects,
                "/v1/recognition/subjects/{subject}", Options);
  ADD_METHOD_TO(RecognitionHandler::searchAppearanceSubject,
   ...
   */
  void
  registerFaceSubject(const HttpRequestPtr &req,
                      std::function<void(const HttpResponsePtr &)> &&callback);
   ...
   */
  void
  listFaceSubjects(const HttpRequestPtr &req,
                   std::function<void(const HttpResponsePtr &)> &&callback);
   ...
   */
  void
  deleteFaceSubject(const HttpRequestPtr &req,
                    std::function<void(const HttpResponsePtr &)> &&callback);
   ...
   */
  void deleteMultipleFaceSubjects(
   ...
   */
  void deleteAllFaceSubjects(
      const HttpRequestPtr &req,
      std::function<void(const HttpResponsePtr &)> &&callback);

  /**
   * @brief Handle PUT /v1/recognition/subjects/{subject}
   * Renames an existing subject. If the new subject name already exists,
   * subjects are merged.
   */
   ...
   */
  void
  handleOptionsFaces(const HttpRequestPtr &req,
                     std::function<void(const HttpResponsePtr &)> &&callback);

  /**
   * @brief Handle OPTIONS request for CORS preflight (subjects endpoint)
   */
  void handleOptionsSubjects(
      const HttpRequestPtr &req,
      std::function<void(const HttpResponsePtr &)> &&callback);
   ...
   */
  void handleOptionsDeleteFaces(
      const HttpRequestPtr &req,
      std::function<void(const HttpResponsePtr &)> &&callback);

  /**
   * @brief Handle OPTIONS request for CORS preflight (delete all faces
   * endpoint)
   */
  void handleOptionsDeleteAll(
      const HttpRequestPtr &req,
      std::function<void(const HttpResponsePtr &)> &&callback);
...
private:
   ...
   */
  bool extractImageData(const HttpRequestPtr &req,
   ...
   */
  bool decodeBase64(const std::string &base64Str,
   ...
   */
  bool extractImageFromJson(const HttpRequestPtr &req,
                            std::vector<unsigned char> &imageData,
                            std::string &error) const;
   ...
   * Max size: 5MB
   */
  bool validateImageFormatAndSize(const std::vector<unsigned char> &imageData,
                                  std::string &error) const;
  ...
  /**
   * @brief Extract image data from request (supports both JSON base64 and
   * multipart/form-data)
   */
  bool extractImageFromRequest(const HttpRequestPtr &req,
                               std::vector<unsigned char> &imageData,
                               std::string &error) const;
  ...
  bool registerSubject(const std::string &subjectName,
                       ...
                       double detProbThreshold, std::string &imageId,
                       std::string &error) const;

  /**
   * @brief Get list of face subjects with pagination
   */
  Json::Value getFaceSubjects(int page, int size,
  ...
  getSubjectImageIds(const std::string &subjectName) const;

  /**
   * @brief Add image ID to subject
   */
  void addImageToSubject(const std::string &subjectName,
   ...
   */
  void mergeSubjects(const std::string &oldSubjectName,
                     const std::string &newSubjectName) const;

  /**
   * @brief Rename subject (move all faces to new name)
   */
  void renameSubjectInStorage(const std::string &oldSubjectName,
                              const std::string &newSubjectName) const;
   ...
   */
  bool removeImageFromSubject(const std::string &subjectName,
                              const std::string &imageId) const;
