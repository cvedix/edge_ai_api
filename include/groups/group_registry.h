...
public:
     ...
     */
    bool registerGroup(const std::string& groupId, 
     ...
     * @param groupId Group ID
     * @return true if successful, false if group not found, is default, or has instances
     */
     ...
     */
    void setInstanceIds(const std::string& groupId, const std::vector<std::string>& instanceIds);
...
private:
    ...
    /**
     * @brief Update instance count without locking (assumes mutex is already locked)
     * @param groupId Group ID
     * @param count New instance count
     */
    ...
    std::map<std::string, std::vector<std::string>> group_instances_; // groupId -> instanceIds
};
