...
public:
     ...
     * @param workerExecutable Path to worker executable (default: "edge_ai_worker")
     */
    SubprocessInstanceManager(SolutionRegistry& solutionRegistry,
    ...
    bool startInstance(const std::string& instanceId, bool skipAutoStop = false) override;
    bool stopInstance(const std::string& instanceId) override;
    bool updateInstance(const std::string& instanceId, const Json::Value& configJson) override;
    
    // ========== Instance Query ==========
    
    std::optional<InstanceInfo> getInstance(const std::string& instanceId) const override;
    ...
    std::optional<InstanceStatistics> getInstanceStatistics(const std::string& instanceId) override;
    ...
    bool updateInstanceFromConfig(const std::string& instanceId, const Json::Value& configJson) override;
...
private:
     ...
     */
    void updateInstanceCache(const std::string& instanceId, const worker::IPCMessage& response);
