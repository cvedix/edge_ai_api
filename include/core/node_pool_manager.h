...
public:
  ...
  struct NodeTemplate {
    ...
    std::string category;    // Category: "source", "detector", "processor",
                             // "destination", "broker"
    std::map<std::string, std::string> defaultParameters; // Default parameters
    std::vector<std::string>
        requiredParameters; // Required parameters that must be provided
   ...
   */
  std::vector<NodeTemplate>
  getTemplatesByCategory(const std::string &category) const;
   ...
   */
  std::optional<PreConfiguredNode>
  getPreConfiguredNode(const std::string &nodeId) const;
  ...
  buildSolutionFromNodes(const std::vector<std::string> &nodeIds,
   ...
   * Creates nodes for all templates that can be created (have all required
   * parameters or defaults) Only creates nodes for types that don't already
   * exist
...
private:
  ...
  createNodeInstance(const NodeTemplate &nodeTemplate,
                     const std::map<std::string, std::string> &parameters);
