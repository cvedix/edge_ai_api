 ...
 * @param max_days Maximum number of days to keep log files (0 = keep forever,
 * default: 30)
 * @param roll_at_hour Hour of day to roll log file (0-23, default: 0 =
 * midnight)
 * @param enable_console Whether to also log to console (default: true)
 */
inline void
init(const std::string &log_dir = "", plog::Severity log_level = plog::info,
     int max_days = 30,
     int /*roll_at_hour*/ =
         0, // Parameter name commented to avoid unused-parameter warning
     bool enable_console = true) {

  // Get log directory from environment or use default
  std::string log_directory =
      log_dir.empty() ? EnvConfig::getString("LOG_DIR", "./logs") : log_dir;
  ...
  int max_files =
      max_days > 0 ? max_days : 0; // Use max_days as max_files count
  ...
  PLOG_INFO << "Log file pattern: " << log_file_path
            << " (will be: log.txt, log.txt.1, log.txt.2, ...)";
  PLOG_INFO << "Log level: " << plog::severityToString(log_level);
  PLOG_INFO << "Max file size: " << (max_file_size / (1024 * 1024)) << "MB";
  PLOG_INFO << "Max files to keep: "
            << (max_files == 0 ? "unlimited" : std::to_string(max_files));
 ...
 * RollingFileAppender creates files with pattern: log.txt, log.txt.1,
 * log.txt.2, etc.
 *
