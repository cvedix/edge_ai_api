 ...
 */
inline bool isApiLoggingEnabled() {
    return g_log_api.load();
}

/**
 * @brief Check if instance execution logging is enabled
 */
inline bool isInstanceLoggingEnabled() {
    return g_log_instance.load();
}
 ...
 */
inline bool isSdkOutputLoggingEnabled() {
    return g_log_sdk_output.load();
}
