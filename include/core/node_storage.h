 ...
 * Nodes created from default solutions are stored, as well as user-created
 * nodes.
 */
class NodeStorage {
public:
   ...
   */
  bool
  saveAllNodes(const std::vector<NodePoolManager::PreConfiguredNode> &nodes);
...
private:
  ...
  jsonToNode(const Json::Value &json) const;
};
