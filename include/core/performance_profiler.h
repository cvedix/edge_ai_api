...
public:
    ...
    class ScopedLock {
    public:
        ScopedLock(LockProfiler& profiler, const std::string& lock_name, std::mutex& mtx)
    ...
    void recordWait(const std::string& lock_name, uint64_t wait_time_us) {
        ...
        while (wait_time_us > current_max && 
               !stats.max_wait_time_us.compare_exchange_weak(current_max, wait_time_us)) {
            // Retry
        }
    }
...
public:
    ...
    void recordFrame(const std::string& stage_name, uint64_t processing_time_us) {
        ...
        while (processing_time_us > current_max && 
               !stats.max_processing_time_us.compare_exchange_weak(current_max, processing_time_us)) {
            // Retry
        }
        ...
        while (processing_time_us < current_min && 
               !stats.min_processing_time_us.compare_exchange_weak(current_min, processing_time_us)) {
            // Retry
        }
    }
...
public:
    ...
    void recordIO(const std::string& io_name, uint64_t time_us, size_t bytes, bool error) {
...
public:
    ...
    void start() {
        cpu_profiler_.startMonitoring();
    }
    
    void stop() {
        cpu_profiler_.stopMonitoring();
    }
        ...
        if (frame_stats.empty()) {
            ...
            for (const auto& [stage, stats] : frame_stats) {
                ...
                if (frames > 0) {
                    ...
                    oss << "    Avg processing time: " << std::fixed << std::setprecision(2) 
                        << avg_time_ms << " ms (" << fps << " FPS)\n";
                    oss << "    Min/Max: " << min_time_ms << " / " << max_time_ms << " ms\n";
                    oss << "    Dropped frames: " << dropped << "\n";
                    
                    if (avg_time_ms > 33.0) { // > 30 FPS threshold
                        oss << "    ⚠️  BOTTLENECK: Slow frame processing (target: < 33ms for 30 FPS)\n";
                    }
        ...
        if (io_stats.empty()) {
            ...
            for (const auto& [io_name, stats] : io_stats) {
                ...
                if (ops > 0) {
                    ...
                    double max_time_ms = max_time / 1000.0;
                    double throughput_mbps = (bytes * 8.0) / (total_time / 1000000.0) / 1000000.0;
