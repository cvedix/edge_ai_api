...
protected:
    ...
    InstanceInfo createValidInstanceInfo(const std::string& instanceId = "test-instance-123") {
        InstanceInfo info;
    ...
    Json::Value createValidConfigJson(const std::string& instanceId = "test-instance-123") {
        ...
        input["uri"] = "gstreamer:///urisourcebin uri=rtsp://localhost:8554/stream ! decodebin ! videoconvert ! video/x-raw, format=NV12 ! appsink drop=true name=cvdsink";
        config["Input"] = input;
...
TEST_F(InstanceUpdateTest, UpdateInstanceRequest_Validate_InvalidName) {
    UpdateInstanceRequest req;
    req.name = "Invalid@Name!"; // Contains invalid characters
    
    EXPECT_FALSE(req.validate());
    EXPECT_FALSE(req.getValidationError().empty());
}

TEST_F(InstanceUpdateTest, UpdateInstanceRequest_Validate_InvalidFrameRateLimit) {
    UpdateInstanceRequest req;
...
TEST_F(InstanceUpdateTest, UpdateInstanceRequest_Validate_InvalidDetectionSensitivity) {
    UpdateInstanceRequest req;
...
TEST_F(InstanceUpdateTest, MergeConfigs_UpdateDetectorPartial) {
    ...
    EXPECT_TRUE(merged);
    EXPECT_EQ(existing["Detector"]["current_preset"].asString(), "FullRegionInference");
    EXPECT_EQ(existing["Detector"]["person_confidence_threshold"].asDouble(), 0.5);
    // Other detector fields should remain unchanged
    EXPECT_EQ(existing["Detector"]["current_sensitivity_preset"].asString(), "Medium");
    EXPECT_EQ(existing["Detector"]["animal_confidence_threshold"].asDouble(), 0.3);
}
...
TEST_F(InstanceUpdateTest, MergeConfigs_UpdateOutputHandlers) {
    ...
    EXPECT_EQ(existing["Output"]["handlers"]["rtsp:--0.0.0.0:8554-stream1"]["uri"].asString(), "rtsp://output:8554/stream");
}
...
TEST_F(InstanceUpdateTest, MergeConfigs_PreserveZoneWhenNotInUpdate) {
    ...
    EXPECT_EQ(existing["Zone"]["Zones"]["zone-123"]["name"].asString(), "Existing Zone");
...
TEST_F(InstanceUpdateTest, MergeConfigs_UpdateZoneWhenInUpdate) {
    ...
    EXPECT_TRUE(merged);
    EXPECT_EQ(existing["Zone"]["Zones"]["zone-123"]["name"].asString(), "New Zone");
...
TEST_F(InstanceUpdateTest, RoundTrip_UpdateInput) {
    ...
    // Update input URI
    config["Input"]["uri"] = "gstreamer:///urisourcebin uri=rtsp://new:8554/stream ! decodebin ! videoconvert ! video/x-raw, format=NV12 ! appsink drop=true name=cvdsink";
...
TEST_F(InstanceUpdateTest, RoundTrip_UpdateOutput) {
    ...
    // Update output
    config["Output"]["JSONExport"]["enabled"] = true;
    config["Output"]["handlers"]["rtsp:--0.0.0.0:8554-stream1"]["config"]["fps"] = 20;
