...
protected:
    void SetUp() override {
        ...
        test_storage_dir_ = std::filesystem::temp_directory_path() / "test_solutions";
...
TEST_F(SolutionHandlerTest, CreateSolutionSuccess) {
    auto req = HttpRequest::newHttpRequest();
    req->setPath("/v1/core/solutions");
    req->setMethod(Post);
...
TEST_F(SolutionHandlerTest, UpdateSolutionSuccess) {
    auto req = HttpRequest::newHttpRequest();
    req->setPath("/v1/core/solutions/test_solution");
    req->setMethod(Put);
