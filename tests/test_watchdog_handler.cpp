...
TEST_F(WatchdogHandlerTest, WatchdogEndpointReturnsValidJson) {
    ...
    // Check required fields - response has "watchdog" and "health_monitor" objects
    EXPECT_TRUE(json->isMember("watchdog"));
    EXPECT_TRUE(json->isMember("health_monitor"));
    
    // Check watchdog object structure
    if ((*json)["watchdog"].isObject() && !(*json)["watchdog"].isMember("error")) {
    ...
    if ((*json)["health_monitor"].isObject() && !(*json)["health_monitor"].isMember("error")) {
