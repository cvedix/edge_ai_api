...
TEST_F(InstanceStorageTest, InstanceInfoToConfigJson_WithRTSPUrl) {
    ...
    EXPECT_NE(config["Input"]["uri"].asString().find("rtsp://localhost:8554/stream"), std::string::npos);
}
...
TEST_F(InstanceStorageTest, ConfigJsonToInstanceInfo_Valid) {
    ...
    EXPECT_EQ(info->frameRateLimit, config["SolutionManager"]["frame_rate_limit"].asInt());
}

TEST_F(InstanceStorageTest, ConfigJsonToInstanceInfo_WithRTSPUrl) {
    Json::Value config = createValidConfigJson();
    config["Input"]["media_type"] = "IP Camera";
    config["Input"]["uri"] = "gstreamer:///urisourcebin uri=rtsp://localhost:8554/stream ! decodebin ! videoconvert ! video/x-raw, format=NV12 ! appsink drop=true name=cvdsink";
    std::string error;
...
TEST_F(InstanceStorageTest, LoadAllInstances) {
    ...
    EXPECT_NE(std::find(allInstances.begin(), allInstances.end(), "instance-1"), allInstances.end());
    EXPECT_NE(std::find(allInstances.begin(), allInstances.end(), "instance-2"), allInstances.end());
}
