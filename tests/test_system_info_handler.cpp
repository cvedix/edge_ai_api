...
protected:
    void SetUp() override {
        handler_ = std::make_unique<SystemInfoHandler>();
    }
...
TEST_F(SystemInfoHandlerTest, GetSystemStatusReturnsValidJson) {
    auto req = HttpRequest::newHttpRequest();
...
TEST_F(SystemInfoHandlerTest, SystemStatusHasValidMemoryInfo) {
    auto req = HttpRequest::newHttpRequest();
    req->setPath("/v1/core/system/status");
    req->setMethod(Get);
