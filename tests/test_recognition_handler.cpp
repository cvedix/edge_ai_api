...
TEST_F(RecognitionHandlerTest, RenameSubjectSuccess) {
...
TEST_F(RecognitionHandlerTest, RenameSubjectMissingApiKey) {
    bool callbackCalled = false;
    HttpResponsePtr response;
...
TEST_F(RecognitionHandlerTest, RenameSubjectUrlEncoded) {
    bool callbackCalled = false;
    HttpResponsePtr response;
...
TEST_F(RecognitionHandlerTest, RenameSubjectCorsHeaders) {
    ...
    EXPECT_TRUE(response->getHeader("Access-Control-Allow-Methods").find("PUT") != std::string::npos);
    EXPECT_TRUE(response->getHeader("Access-Control-Allow-Headers").find("x-api-key") != std::string::npos);
}
...
TEST_F(RecognitionHandlerTest, DeleteFaceSubjectSuccess) {
    ...
    registerBody["file"] = "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mNk+M9QDwADhgGAWjR9awAAAABJRU5ErkJggg==";
...
TEST_F(RecognitionHandlerTest, DeleteFaceSubjectMissingApiKey) {
    bool callbackCalled = false;
    HttpResponsePtr response;
...
TEST_F(RecognitionHandlerTest, DeleteMultipleFaceSubjectsMissingApiKey) {
    bool callbackCalled = false;
    HttpResponsePtr response;
...
// Test deleteMultipleFaceSubjects endpoint with non-existent IDs (should ignore them)
