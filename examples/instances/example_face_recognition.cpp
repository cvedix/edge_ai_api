 ...
 *
 *   Mode: register
 *     ./insightface_register_recognize_face_sample register <image_path>
 * <person_name> [onnx_model_path] Ví dụ:
 * ./insightface_register_recognize_face_sample register alice.jpg "Alice"
 *
 *   Mode: recognize
 *     ./insightface_register_recognize_face_sample recognize
 * [video_path|image_path] [onnx_model_path] Ví dụ:
 * ./insightface_register_recognize_face_sample recognize face.mp4 Ví dụ:
 * ./insightface_register_recognize_face_sample recognize photo.jpg
 *
 * Database:
 ...
 *
 * Yêu cầu:
 *   - ONNX model: face_recognition_sface_2021dec.onnx hoặc model InsightFace
 * khác
...
static cv::Mat align_face_using_landmarks(const cv::Mat &image,
  ...
  // rcm_y, lcm_x, lcm_y, score) Landmarks order: right_eye, left_eye, nose_tip,
...
extract_embedding_from_image(const cv::Mat &aligned_face,
  ...
  if (net.empty()) {
    std::cerr << "[Error] Failed to load ONNX model: " << onnx_model_path
              << std::endl;
    return std::vector<float>();
  }
...
static std::string get_project_root(const std::string &executable_path) {
  ...
  for (int depth = 0; depth < 5; depth++) {
    if (current.filename() == "bin" &&
        current.parent_path().filename() == "build") {
...
static std::string resolve_path(const std::string &executable_path,
...
private:
  ...
  std::string resolve_model_path(const std::string &relative_path) {
    std::filesystem::path full_path =
        std::filesystem::path(project_root_) / relative_path;
    if (std::filesystem::exists(full_path))
      return full_path.string();
...
public:
  FaceDatabase(const std::string &executable_path,
               const std::string &db_path = "./face_database.txt")
    ...
    std::vector<std::string> model_paths = {
        resolve_path(executable_path, "build/bin/cvedix_data/models/face/"
                                      "face_recognition_sface_2021dec.onnx"),
        resolve_path(
            executable_path,
            "cvedix_data/models/face/face_recognition_sface_2021dec.onnx"),
        (std::filesystem::path(project_root_) /
         "build/bin/cvedix_data/models/face/"
         "face_recognition_sface_2021dec.onnx")
            .string(),
        (std::filesystem::path(project_root_) /
         "cvedix_data/models/face/face_recognition_sface_2021dec.onnx")
    ...
    if (onnx_model_path_.empty()) {
      std::cerr << "[DB] Warning: ONNX model not found, using default path"
                << std::endl;
  ...
  bool register_face_from_image(const std::string &image_path,
                                const std::string &person_name) {
    std::cout << "\n[Register] Image: " << image_path
    ...
    std::vector<std::string> detector_paths = {
        resolve_model_path("build/bin/cvedix_data/models/face/"
    ...
    try {
      ...
      std::cerr << "[Register] Error: Failed to create detector: " << e.what()
                << std::endl;
      return false;
    }
    ...
    try {
      ...
      std::cerr << "[Register] Error: Detection failed: " << e.what()
                << std::endl;
      return false;
    }
    ...
    if (embeddings.empty()) {
      std::cerr << "[Register] Error: Failed to extract any embeddings"
                << std::endl;
      return false;
    }
    ...
    std::cout << "[Register] Generated " << embeddings.size()
              << " embeddings, averaged to final embedding" << std::endl;

    database_[person_name] = final_embedding;
    save_database();
    std::cout << "[Register] ✓ Registered: " << person_name << " (using "
              << embeddings.size() << " augmented variations)" << std::endl;
    return true;
  }
  ...
  std::string identify(const std::vector<float> &query_embedding) {
    ...
    std::cout << "  [Debug] Query embedding size: " << query_embedding.size()
              << std::endl;
    if (!database_.empty()) {
      auto first_entry = database_.begin();
      std::cout << "  [Debug] Database embedding size: "
                << first_entry->second.size() << std::endl;

      if (query_embedding.size() != first_entry->second.size()) {
        std::cerr << "  [Error] Embedding size mismatch! Query: "
                  << query_embedding.size()
                  << ", Database: " << first_entry->second.size() << std::endl;
        std::cerr << "  [Error] This usually means using different models "
                     "(ONNX vs TRT) or different model versions."
                  << std::endl;
        std::cerr << "  [Error] Solution: Re-register all faces using the same "
                     "model that's being used for recognition."
                  << std::endl;
        return "Unknown";
      }
    }
    ...
    if (confidence_gap < 0.1 && best_match != "Unknown") {
      std::cout << "  [Warning] Low confidence gap (" << std::fixed
                << std::setprecision(4) << confidence_gap << ") between "
                << best_match << " (" << std::fixed << std::setprecision(4)
                << best_sim << ") and " << second_match << " (" << std::fixed
                << std::setprecision(4) << second_sim << ")" << std::endl;
      std::cout
          << "  [Result] Rejecting match due to ambiguous similarity scores"
          << std::endl;
      return "Unknown";
    }

    // Additional check: best similarity must be significantly higher than
    // threshold Require at least 0.15 above threshold to ensure confident match
    float min_required_sim = threshold_ + 0.15f;
    if (best_sim < min_required_sim) {
      std::cout << "  [Warning] Best similarity (" << std::fixed
                << std::setprecision(4) << best_sim
                << ") is too close to threshold (" << threshold_ << ")"
                << std::endl;
      std::cout << "  [Result] Requiring similarity >= " << std::fixed
                << std::setprecision(4) << min_required_sim << std::endl;
      return "Unknown";
    }
...
int recognize_image(const std::string &executable_path,
                    const std::string &image_path,
                    const std::string &onnx_model_path);

void face_recognition_callback(
  ...
  if (!frame_meta || frame_meta->face_targets.empty() || !g_database)
    return;
...
int recognize_image(const std::string &executable_path,
  ...
  std::cout << "[Image Recognition] Using detector: " << detector_model_path
            << std::endl;
  ...
  for (int i = 0; i < faces.rows; i++) {
    ...
    // Original
    std::vector<float> emb1 =
        extract_embedding_from_image(aligned_face, onnx_model_path);
    if (!emb1.empty())
      embeddings.push_back(emb1);
    ...
    cv::flip(aligned_face, flipped, 1);
    std::vector<float> emb2 =
        extract_embedding_from_image(flipped, onnx_model_path);
    if (!emb2.empty())
      embeddings.push_back(emb2);

    if (embeddings.empty()) {
      std::cerr << "  [Error] Failed to extract embedding for face " << (i + 1)
                << std::endl;
      continue;
    }
    ...
    std::vector<float> final_embedding = average_embeddings(embeddings);
    std::cout << "  [Debug] Extracted embedding size: "
              << final_embedding.size() << " (from " << embeddings.size()
              << " variations)" << std::endl;
    std::string person_name = g_database->identify(final_embedding);
    std::cout << "  Face " << (i + 1) << ": (" << (int)x << "," << (int)y
              << ") " << (int)w << "x" << (int)h << " -> " << person_name
              << std::endl;

    cv::rectangle(result_image, cv::Rect((int)x, (int)y, (int)w, (int)h),
                  cv::Scalar(0, 255, 0), 2);
...
int mode_register(int argc, char *argv[]) {
  if (argc < 4) {
    std::cerr << "Usage: " << argv[0]
              << " register <image_path> <person_name> [onnx_model_path]"
              << std::endl;
    return 1;
  }
...
int mode_recognize(int argc, char *argv[]) {
  std::string input_path =
      (argc >= 3) ? argv[2] : "./cvedix_data/test_video/face.mp4";
  ...
  if (!std::filesystem::exists(resolved_input_path)) {
    std::cerr << "\n[Error] Input file not found: " << resolved_input_path
              << std::endl;
    return 1;
  }
  ...
  if (g_database->size() == 0) {
    std::cerr << "\n[Error] Database is empty! Register faces first."
              << std::endl;
    std::cerr << "Usage: " << argv[0] << " register <image_path> <person_name>"
              << std::endl;
    return 1;
  }
  ...
  std::cout << "\nConfig: Input=" << resolved_input_path
            ...
            << ", DB=" << g_database->size()
            << " faces, Threshold=" << g_database->threshold() << "\n"
            << std::endl;
  ...
  if (is_image) {
    ...
    return recognize_image(argv[0], resolved_input_path,
                           resolve_path(argv[0], model_path));
  }

  std::string project_root = get_project_root(argv[0]);
  std::vector<std::string> detector_paths = {
      resolve_path(argv[0], "build/bin/cvedix_data/models/face/"
                            "face_detection_yunet_2023mar_int8.onnx"),
      resolve_path(
          argv[0],
          "cvedix_data/models/face/face_detection_yunet_2023mar_int8.onnx"),
  ...
  std::vector<std::string> model_paths = {
      ...
      resolve_path(
          ...
          "cvedix_data/models/face/face_recognition_sface_2021dec.onnx"),
      (std::filesystem::path(project_root) /
       "build/bin/cvedix_data/models/face/face_recognition_sface_2021dec.onnx")
          .string(),
      (std::filesystem::path(project_root) /
       "cvedix_data/models/face/face_recognition_sface_2021dec.onnx")
          .string()};
  ...
  if (!onnx_model_path.empty()) {
    model_paths.insert(model_paths.begin(),
                       resolve_path(argv[0], onnx_model_path));
  }
  ...
  if (!found_model) {
    std::cerr << "[Error] Face recognition model not found. Tried:"
              << std::endl;
    for (const auto &path : model_paths) {
      std::cerr << "  - " << path << std::endl;
    }
  ...
  std::cout << "[Video Recognition] Using detector: " << detector_model
            << std::endl;
  std::cout << "[Video Recognition] Using recognizer: " << onnx_model
            << std::endl;

  auto file_src = std::make_shared<cvedix_nodes::cvedix_file_src_node>(
      "file_src", 0, resolved_input_path, 0.6);
  auto detector =
      std::make_shared<cvedix_nodes::cvedix_yunet_face_detector_node>(
          "detector", detector_model, 0.9f, 0.3f, 5000);
  auto recognizer =
      std::make_shared<cvedix_nodes::cvedix_insight_face_recognition_node>(
          "recognizer", onnx_model, 112, 112, true);
...
int main(int argc, char *argv[]) {
  if (argc < 2) {
    ...
    std::cout << "  Register: " << argv[0]
              << " register <image_path> <person_name> [onnx_model_path]"
              << std::endl;
    std::cout << "  Recognize: " << argv[0]
              << " recognize [video_path|image_path] [onnx_model_path]"
              << std::endl;
    std::cout << "\nExamples:" << std::endl;
    std::cout << "  " << argv[0] << " register alice.jpg \"Alice\""
              << std::endl;
  ...
  if (mode == "register")
    return mode_register(argc, argv);
  if (mode == "recognize")
    return mode_recognize(argc, argv);
...
#include <vector>
  ...
  // Helper function: Average embeddings
  static std::vector<float> average_embeddings(
  ...
  // Helper function: Face alignment using landmarks (if available)
  static cv::Mat align_face_using_landmarks(
      const cv::Mat &image, const cv::Mat &faces, int face_idx) {
    // YuNet format: (x, y, w, h, re_x, re_y, le_x, le_y, nt_x, nt_y, rcm_x,
    // rcm_y, lcm_x, lcm_y, score) Landmarks order: right_eye, left_eye,
    // nose_tip, right_mouth_corner, left_mouth_corner
  ...
  static std::vector<float> extract_embedding_from_image(
      const cv::Mat &aligned_face, const std::string &onnx_model_path) {
  ...
  private:
    ...
    void save_database() {
      std::ofstream file(db_file_path_);
      if (!file.is_open()) {
        std::cerr << "[DB] Error: Cannot save to " << db_file_path_
                  << std::endl;
        return;
      }
    ...
    bool register_face_from_image(const std::string &image_path,
      ...
      if (faces.cols >= 15) { // YuNet has landmarks
        ...
        cv::Mat face_roi =
            image(cv::Rect((int)x, (int)y, (int)w, (int)h)).clone();
        cv::resize(face_roi, aligned_face, cv::Size(112, 112));
        std::cout << "[Register] Using simple resize (no landmarks)"
                  << std::endl;
      }
    ...
    std::string identify(const std::vector<float> &query_embedding) {
      ...
      if (!database_.empty()) {
        ...
        if (query_embedding.size() != first_entry->second.size()) {
          std::cerr << "  [Error] Embedding size mismatch! Query: "
                    << query_embedding.size()
                    << ", Database: " << first_entry->second.size()
                    << std::endl;
          std::cerr << "  [Error] This usually means using different models "
                       "(ONNX vs TRT) or different model versions."
                    << std::endl;
          std::cerr << "  [Error] Solution: Re-register all faces using the "
                       "same model that's being used for recognition."
                    << std::endl;
          return "Unknown";
        }
      }
      ...
      // Additional check: best similarity must be significantly higher than
      // threshold Require at least 0.15 above threshold to ensure confident
      // match
      float min_required_sim = threshold_ + 0.15f;
      if (best_sim < min_required_sim) {
        std::cout << "  [Warning] Best similarity (" << std::fixed
  ...
  int recognize_image(const std::string &executable_path,
                      const std::string &image_path,
                      const std::string &onnx_model_path) {
    std::cout << "\n[Image Recognition] Processing: " << image_path
              << std::endl;
    ...
    try {
      ...
      std::cerr << "[Error] Failed to create detector: " << e.what()
                << std::endl;
      return 1;
    }
    ...
    for (int i = 0; i < faces.rows; i++) {
      ...
      // Extract embedding with augmentation (original + flip) for better
      // accuracy
      std::vector<std::vector<float>> embeddings;
      ...
      if (embeddings.empty()) {
        std::cerr << "  [Error] Failed to extract embedding for face "
                  << (i + 1) << std::endl;
        continue;
      }
  ...
  int mode_recognize(int argc, char *argv[]) {
    ...
    if (g_database->size() == 0) {
      std::cerr << "\n[Error] Database is empty! Register faces first."
                << std::endl;
      std::cerr << "Usage: " << argv[0]
                << " register <image_path> <person_name>" << std::endl;
      return 1;
    }
