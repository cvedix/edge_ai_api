 ...
 *
 * Pipeline:
 *   RTSP Source -> YOLO Detector -> SORT Tracker -> Crossline BA -> Crossline OSD -> Screen/RTMP
 *
 * Yêu cầu:
...
int main() {
    ...
    try {
        ...
        if (!display_available) {
            std::cerr << "[WARN] DISPLAY/WAYLAND not found. Screen DES node will be skipped." << std::endl;
        }
        if (!textoverlay_available) {
            std::cerr << "[WARN] GStreamer element 'textoverlay' not found. Screen DES node requires this plugin and will be disabled." << std::endl;
        }
        ...
        if (textoverlay_available) {
            rtmp_des_0 = std::make_shared<cvedix_nodes::cvedix_rtmp_des_node>("rtmp_des_0", 0, rtmp_url);
        } else {
        ...
        if (screen_des_0) {
            std::cout << "Pipeline started. Screen DES will display the stream locally." << std::endl;
        } else {
            std::cout << "Pipeline started without local display (screen disabled)." << std::endl;
        }
