...
bool UpdateInstanceRequest::validate() const {
    ...
    if (!name.empty()) {
        ...
        if (!std::regex_match(name, namePattern)) {
            validation_error_ = "Invalid name format. Name must match pattern: ^[A-Za-z0-9 -_]+$";
            return false;
        }
    }
    ...
    if (!group.empty()) {
        ...
        if (!std::regex_match(group, groupPattern)) {
            validation_error_ = "Invalid group format. Group must match pattern: ^[A-Za-z0-9 -_]+$";
            return false;
        }
    }
    ...
    if (!detectionSensitivity.empty()) {
        if (detectionSensitivity != "Low" && detectionSensitivity != "Medium" && detectionSensitivity != "High") {
    ...
    if (!movementSensitivity.empty()) {
        if (movementSensitivity != "Low" && movementSensitivity != "Medium" && movementSensitivity != "High") {
    ...
    if (inputOrientation != -1 && (inputOrientation < 0 || inputOrientation > 3)) {
...
bool UpdateInstanceRequest::hasUpdates() const {
    return !name.empty() ||
           !group.empty() ||
           persistent.has_value() ||
           frameRateLimit != -1 ||
           metadataMode.has_value() ||
           statisticsMode.has_value() ||
           diagnosticsMode.has_value() ||
           debugMode.has_value() ||
           !detectorMode.empty() ||
           !detectionSensitivity.empty() ||
           !movementSensitivity.empty() ||
           !sensorModality.empty() ||
           autoStart.has_value() ||
           autoRestart.has_value() ||
           inputOrientation != -1 ||
           inputPixelLimit != -1 ||
