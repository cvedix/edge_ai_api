...
namespace GStreamerChecker {
    ...
    std::map<std::string, PluginInfo> checkRequiredPlugins() {
        ...
        plugins["qtdemux"] = {"qtdemux", "MP4 demuxer (for video files)", "gstreamer1.0-plugins-good", true, false};
        plugins["h264parse"] = {"h264parse", "H.264 parser", "gstreamer1.0-plugins-good", true, false};
        plugins["avdec_h264"] = {"avdec_h264", "H.264 decoder (libav)", "gstreamer1.0-libav", true, false};
        
        // Required plugins for RTMP output
        plugins["x264enc"] = {"x264enc", "H.264 encoder (x264)", "gstreamer1.0-plugins-ugly", true, false};
        
        // Alternative encoders (optional but useful)
        plugins["openh264enc"] = {"openh264enc", "H.264 encoder (OpenH264)", "gstreamer1.0-plugins-bad", false, false};
        
        // RTMP plugins
        plugins["flvmux"] = {"flvmux", "FLV muxer (for RTMP)", "gstreamer1.0-plugins-good", true, false};
        plugins["rtmpsink"] = {"rtmpsink", "RTMP sink", "gstreamer1.0-plugins-bad", true, false};
        
        // Common plugins
        plugins["videoconvert"] = {"videoconvert", "Video format converter", "gstreamer1.0-plugins-base", true, false};
        plugins["appsink"] = {"appsink", "Application sink", "gstreamer1.0-plugins-base", true, false};
        plugins["appsrc"] = {"appsrc", "Application source", "gstreamer1.0-plugins-base", true, false};
    ...
    std::string getInstallationCommand(const std::vector<std::string>& missingPlugins) {
        ...
        for (const auto& [name, info] : plugins) {
            if (info.required) {
                if (!info.available) {
                    ...
                    std::cerr << "[GStreamerChecker]   Package: " << info.package << std::endl;
                } else if (verbose) {
    ...
    bool validatePlugins(bool autoSuggest) {
        ...
        if (!missingRequired.empty()) {
            ...
            if (autoSuggest) {
                std::string installCmd = getInstallationCommand(missingRequired);
                if (!installCmd.empty()) {
                    std::cerr << "[GStreamerChecker] NOTE: You can install missing plugins with:" << std::endl;
                    std::cerr << "[GStreamerChecker]   " << installCmd << std::endl;
                }
            }
