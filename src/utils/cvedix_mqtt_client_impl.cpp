...
cvedix_mqtt_client::cvedix_mqtt_client(
    ...
    , port_(port)
    , client_id_(client_id.empty() ? "cvedix_mqtt_client_" + std::to_string(time(nullptr)) : client_id)
...
int cvedix_mqtt_client::publish(const std::string& topic, const std::string& payload, int qos, bool retain) {
    ...
    int mid = 0;
    int rc = mosquitto_publish(mosq_, &mid, topic.c_str(), payload.size(), payload.c_str(), qos, retain);
    if (rc != MOSQ_ERR_SUCCESS) {
    ...
    while (!should_stop_reconnect_) {
        ...
        if (!connected_ && !connecting_ && mosq_) {
            ...
            if (rc == MOSQ_ERR_SUCCESS) {
                ...
                connecting_ = false;
                last_error_ = "Reconnect failed: " + std::string(mosquitto_strerror(rc));
            }
        }
    }
}
...
void cvedix_mqtt_client::on_connect_wrapper(struct mosquitto* mosq, void* obj, int rc) {
    ...
    if (rc == 0) {
        ...
        client->connecting_ = false;
        client->last_error_ = "Connection failed: " + std::string(mosquitto_strerror(rc));
        if (client->on_connect_cb_) {
