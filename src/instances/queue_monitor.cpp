...
QueueMonitor::~QueueMonitor() { stopMonitoring(); }

void QueueMonitor::recordQueueFullWarning(const std::string &instanceId,
  ...
  if (count <= 5) {
    std::cerr << "[QueueMonitor] Instance " << instanceId
              << " queue full warning #" << count << " from node: " << nodeName
              << std::endl;
  }

  // Log if threshold exceeded
  if (count > 0 &&
      count % 10 == 0) { // Reduced from 50 to 10 for faster detection
    auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(
                       now - stats.first_warning_time)
                       .count();
    if (elapsed > 0) {
      double rate = static_cast<double>(count) / elapsed;
      std::cerr << "[QueueMonitor] Instance " << instanceId
                << " queue full warnings: " << count << " in " << elapsed
      ...
      if (rate > auto_clear_threshold_) {
        std::cerr << "[QueueMonitor] WARNING: Queue full rate (" << rate
                  ...
                  << "). Consider clearing queue or reducing frame rate."
                  << std::endl;
      }
    }
  }
    ...
    if (elapsed > 0) {
      ...
      if (rate > auto_clear_threshold_) {
        std::cerr << "[QueueMonitor] Queue clearing recommended for instance "
                  << instanceId << " (rate: " << rate << " warnings/s)"
                  << std::endl;
        return true;
      }
    }
...
QueueMonitor::getStats(const std::string &instanceId) {
  std::lock_guard<std::mutex> lock(stats_mutex_);
  auto it = instance_stats_.find(instanceId);
  if (it != instance_stats_.end()) {
    return &it->second;
...
void QueueMonitor::setAutoClearThreshold(double threshold) {
  ...
  std::cerr << "[QueueMonitor] Auto-clear threshold set to " << threshold
            << " warnings/s" << std::endl;
}

void QueueMonitor::setMonitoringWindow(int window) {
  monitoring_window_ = window;
  std::cerr << "[QueueMonitor] Monitoring window set to " << window
            << " seconds" << std::endl;
}
  ...
  while (running_.load()) {
    ...
    for (auto &[instanceId, stats] : instance_stats_) {
      ...
      if (elapsed > 0) {
        ...
        // Log periodic status
        if (count % 100 == 0) {
          std::cerr << "[QueueMonitor] Instance " << instanceId << ": " << count
                    << " warnings in " << elapsed << "s (rate: " << rate
                    << " warnings/s)" << std::endl;
        }

        // Auto-clear recommendation
        if (rate > auto_clear_threshold_ &&
            count >= max_warnings_before_clear_) {
          std::cerr
              << "[QueueMonitor] RECOMMENDATION: Clear queue for instance "
...
std::string QueueMonitor::parseLogLine(const std::string &logLine) {
  ...
  // Node name format: node_type_instanceId (e.g.,
  // yolo_detector_90d86cfb-46fa-4214-8a3c-e7c737ccf369) Extract node name from
  // log line
...
void QueueMonitor::enableLogParsing(const std::string &logFilePath) {
  ...
  std::cerr << "[QueueMonitor] Log parsing enabled for: " << logFilePath
            << std::endl;
}
...
void QueueMonitor::parseLogFile() {
  ...
  while (std::getline(log_file, line)) {
    ...
    if (!instanceId.empty()) {
      ...
      if (std::regex_search(line, nodeMatch, nodePattern) &&
          nodeMatch.size() > 1) {
...
void QueueMonitor::updateNodeToInstanceMap() {
  ...
  // For now, we extract instance ID from node name pattern:
  // node_type_instanceId Example:
  // yolo_detector_90d86cfb-46fa-4214-8a3c-e7c737ccf369 This is a simple
  // approach - can be improved with actual instance registry integration
}
