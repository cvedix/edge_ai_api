...
void AIWebSocketController::handleNewConnection(const HttpRequestPtr& req,
                                                const WebSocketConnectionPtr& wsConnPtr) {
    active_connections_++;
    std::cout << "[WebSocket] New connection. Total: " << active_connections_.load() << std::endl;
...
void AIWebSocketController::handleConnectionClosed(const WebSocketConnectionPtr& wsConnPtr) {
    active_connections_--;
    std::cout << "[WebSocket] Connection closed. Total: " << active_connections_.load() << std::endl;
}

void AIWebSocketController::handleNewMessage(const WebSocketConnectionPtr& wsConnPtr,
                                             std::string&& message,
...
void AIWebSocketController::processStreamMessage(const WebSocketConnectionPtr& wsConnPtr,
    ...
    try {
        ...
        if (!reader.parse(message, json)) {
            ...
            sendResult(wsConnPtr, Json::writeString(Json::StreamWriterBuilder(), error));
            return;
        }
        ...
        if (msg_type == "process") {
            ...
            sendResult(wsConnPtr, Json::writeString(Json::StreamWriterBuilder(), result));
        } else if (msg_type == "ping") {
            Json::Value pong;
            pong["type"] = "pong";
            sendResult(wsConnPtr, Json::writeString(Json::StreamWriterBuilder(), pong));
        }
