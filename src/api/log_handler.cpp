...
void LogHandler::listLogFiles(const HttpRequestPtr & /*req*/,
                                 ...
                                 std::function<void(const HttpResponsePtr &)> &&callback)
{
    try {
        ...
        if (!parseCategory(category_str, category)) {
            callback(createErrorResponse(k400BadRequest, "Bad request", "Invalid category: " + category_str));
            return;
        }
        ...
        if (tail_count > 0 && !files.empty()) {
            std::string latest_file = LogManager::getLogFilePath(category, files[0].first);
...
void LogHandler::getLogsByCategoryAndDate(const HttpRequestPtr &req,
                                         std::function<void(const HttpResponsePtr &)> &&callback)
{
    try {
        ...
        if (date_str.length() != 10 || date_str[4] != '-' || date_str[7] != '-') {
            callback(createErrorResponse(k400BadRequest, "Bad request", "Invalid date format. Expected YYYY-MM-DD"));
            return;
        }
        ...
        if (!fs::exists(file_path)) {
            callback(createErrorResponse(k404NotFound, "Not found", "Log file not found for date: " + date_str));
            return;
        }
        ...
        // Apply filters
        auto filteredLogs = filterLogs(logs, level_filter, from_timestamp, to_timestamp);
...
void LogHandler::handleOptions(const HttpRequestPtr & /*req*/,
                              std::function<void(const HttpResponsePtr &)> &&callback)
{
...
std::vector<Json::Value> LogHandler::filterLogs(const std::vector<Json::Value>& logs,
                                               ...
                                               const std::string& to_timestamp) const
{
    ...
    for (const auto& log : logs) {
        ...
        if (!level_filter.empty()) {
            ...
            std::transform(log_level_upper.begin(), log_level_upper.end(), log_level_upper.begin(), ::toupper);
...
std::vector<std::string> LogHandler::getTailLines(const std::string& file_path, int tail_count) const
{
    ...
    try {
        ...
        // Get last N lines
        int start_idx = std::max(0, static_cast<int>(all_lines.size()) - tail_count);
...
std::vector<Json::Value> LogHandler::readLogFile(const std::string& file_path, int tail_count) const
{
...
bool LogHandler::parseCategory(const std::string& category_str, LogManager::Category& category) const
{
...
time_t LogHandler::parseTimestamp(const std::string& timestamp) const
{
    try {
        ...
        if (sscanf(ts.c_str(), "%d-%d-%dT%d:%d:%d.%d", 
                   &tm.tm_year, &tm.tm_mon, &tm.tm_mday,
                   &tm.tm_hour, &tm.tm_min, &tm.tm_sec, &tm.tm_isdst) >= 6) {
            tm.tm_year -= 1900;
            tm.tm_mon -= 1;
            tm.tm_isdst = -1;
...
HttpResponsePtr LogHandler::createErrorResponse(int statusCode, const std::string& error, const std::string& message) const
{
