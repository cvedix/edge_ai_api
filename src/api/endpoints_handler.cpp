...
void EndpointsHandler::getEndpointsStats(
  ...
  try {
    ...
    for (const auto &pair : allStats) {
      ...
      endpointInfo["request_count"] =
          static_cast<Json::Int64>(stats->request_count.load());
      endpointInfo["error_count"] =
          static_cast<Json::Int64>(stats->error_count.load());

      // Calculate average response time
      uint64_t req_count = stats->request_count.load();
      if (req_count > 0) {
        uint64_t avg_response_time =
            stats->total_response_time_ms.load() / req_count;
        endpointInfo["avg_response_time_ms"] =
            static_cast<Json::Int64>(avg_response_time);
        endpointInfo["max_response_time_ms"] =
            static_cast<Json::Int64>(stats->max_response_time_ms.load());
        endpointInfo["min_response_time_ms"] = static_cast<Json::Int64>(
            stats->min_response_time_ms.load() == UINT64_MAX
                ? 0
                : stats->min_response_time_ms.load());

        // Calculate error rate
        double error_rate =
            static_cast<double>(stats->error_count.load()) / req_count;
