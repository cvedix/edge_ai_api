...
void InstanceHandler::configureStreamOutput(
    ...
    try {
        ...
        if (enabled) {
            // Check if path is provided (for record output) or uri (for stream output)
            ...
            if (hasPath) {
                ...
                if (path.empty()) {
                    if (isApiLoggingEnabled()) {
                        PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error: Path cannot be empty";
                    }
                ...
                try {
                    ...
                    if (!fs::is_directory(dirPath)) {
                        if (isApiLoggingEnabled()) {
                            PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error: Path is not a directory: " << path;
                        }
                        callback(createErrorResponse(400, "Bad request", "Path must be a directory"));
                        return;
                    }
                    ...
                    if (!testStream.is_open()) {
                        if (isApiLoggingEnabled()) {
                            PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error: No write permission for path: " << path;
                        }
                        callback(createErrorResponse(400, "Bad request", "Path does not have write permissions"));
                        return;
                    }
                    testStream.close();
                    ...
                    if (isApiLoggingEnabled()) {
                        PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error validating path: " << e.what();
                    }
                    callback(createErrorResponse(400, "Bad request", "Invalid path: " + std::string(e.what())));
                    return;
                } catch (const std::exception& e) {
                    if (isApiLoggingEnabled()) {
                        PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error validating path: " << e.what();
                    }
                    callback(createErrorResponse(400, "Bad request", "Error validating path: " + std::string(e.what())));
                    return;
                }
                ...
                if (isApiLoggingEnabled()) {
                    PLOG_WARNING << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Error: Missing 'path' or 'uri' field";
                }
                callback(createErrorResponse(400, "Bad request", "Field 'path' (for record output) or 'uri' (for stream output) is required when enabled is true"));
                return;
            }
            
            // Update instance using updateInstanceFromConfig
            // This will merge streamConfig with existing config and update AdditionalParams
            if (instance_registry_->updateInstanceFromConfig(instanceId, streamConfig)) {
                auto end_time = std::chrono::steady_clock::now();
                auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end_time - start_time);
                if (isApiLoggingEnabled()) {
                    PLOG_INFO << "[API] POST /v1/core/instance/" << instanceId << "/output/stream - Success: Record/stream output configured - " << duration.count() << "ms";
                }
            ...
            streamConfig["AdditionalParams"]["RTMP_URL"] = ""; // Empty string to clear
            streamConfig["AdditionalParams"]["RECORD_PATH"] = ""; // Empty string to clear
