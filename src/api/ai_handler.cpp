...
void AIHandler::processImage(const HttpRequestPtr &req,
    ...
    try {
        ...
        if (!rate_limiter_->allow(client_key)) {
            ...
            PerformanceMonitor::getInstance().recordRequest("/v1/core/ai/process", latency, false);
        ...
        if (cache_) {
            ...
            if (cached_result.has_value()) {
                ...
                auto latency = std::chrono::duration_cast<std::chrono::milliseconds>(
                    std::chrono::steady_clock::now() - start_time);
                PerformanceMonitor::getInstance().recordRequest("/v1/core/ai/process", latency, true);
...
void AIHandler::processBatch(const HttpRequestPtr &req,
                             std::function<void(const HttpResponsePtr &)> &&callback) {
...
void AIHandler::getStatus(const HttpRequestPtr &req,
                          std::function<void(const HttpResponsePtr &)> &&callback) {
    Json::Value response;
    ...
    if (resource_manager_) {
        ...
        response["gpu_available"] = static_cast<Json::UInt64>(gpu_stats.available_gpus);
    }
...
void AIHandler::getMetrics(const HttpRequestPtr &req,
                          std::function<void(const HttpResponsePtr &)> &&callback) {
    ...
    if (rate_limiter_) {
        ...
        rate_data["active_keys"] = static_cast<Json::UInt64>(rate_stats.active_keys);
