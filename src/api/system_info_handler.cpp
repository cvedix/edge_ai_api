...
static std::chrono::steady_clock::time_point g_last_cpu_check = std::chrono::steady_clock::now();
static uint64_t g_last_idle_time = 0;
static uint64_t g_last_total_time = 0;

void SystemInfoHandler::getSystemInfo(const HttpRequestPtr & /*req*/,
                                     std::function<void(const HttpResponsePtr &)> &&callback)
{
    try {
        ...
        callback(createErrorResponse(k500InternalServerError, "Internal server error", e.what()));
    }
}

void SystemInfoHandler::getSystemStatus(const HttpRequestPtr & /*req*/,
                                       std::function<void(const HttpResponsePtr &)> &&callback)
{
    try {
        Json::Value response;
...
void SystemInfoHandler::handleOptions(const HttpRequestPtr & /*req*/,
                                    std::function<void(const HttpResponsePtr &)> &&callback)
{
...
Json::Value SystemInfoHandler::getCPUInfo() const
{
    ...
    try {
        ...
        cpuInfo["regular_frequency"] = static_cast<Json::Int>(cpu.regularClockSpeed_MHz());
        ...
        if (!freqs.empty()) {
            ...
            if (sum > 0) {
                cpuInfo["current_frequency"] = static_cast<Json::Int>(sum / static_cast<int64_t>(freqs.size()));
            } else {
...
Json::Value SystemInfoHandler::getRAMInfo() const
{
    ...
    try {
        ...
        if (!modules.empty()) {
            ...
            ramInfo["serial_number"] = module.serial_number.empty() ? "N/A" : module.serial_number;
        } else {
...
Json::Value SystemInfoHandler::getGPUInfo() const
{
    ...
    try {
        ...
        for (const auto& gpu : gpus) {
            ...
            gpuInfo["memory_mib"] = static_cast<Json::Int64>(memoryBytes / (1024 * 1024));
...
Json::Value SystemInfoHandler::getDiskInfo() const
{
    ...
    try {
        ...
        for (const auto& disk : disks) {
            ...
            diskInfo["free_size_bytes"] = static_cast<Json::Int64>(disk.free_size_Bytes());
...
Json::Value SystemInfoHandler::getOSInfo() const
{
    ...
    try {
        ...
        } else if (name.find("Linux") != std::string::npos || name.find("Ubuntu") != std::string::npos) {
            osInfo["short_name"] = "Linux";
        } else if (name.find("macOS") != std::string::npos || name.find("Darwin") != std::string::npos) {
...
Json::Value SystemInfoHandler::getBatteryInfo() const
{
    ...
    try {
        ...
        for (const auto& battery : batteries) {
            ...
            batteryInfo["capacity_mwh"] = static_cast<Json::Int64>(battery.getEnergyFull());
...
double SystemInfoHandler::getCPUUsage() const
{
    try {
        ...
        auto elapsed = std::chrono::duration_cast<std::chrono::milliseconds>(now - g_last_cpu_check).count();
        ...
        if (g_last_total_time > 0 && g_last_idle_time > 0) {
            ...
            if (totalDiff > 0) {
                double usage = (1.0 - static_cast<double>(idleDiff) / static_cast<double>(totalDiff)) * 100.0;
...
Json::Value SystemInfoHandler::getMemoryStatus() const
{
    ...
    try {
        ...
        memStatus["total_mib"] = static_cast<Json::Int64>(totalBytes / (1024 * 1024));
        ...
        memStatus["available_mib"] = static_cast<Json::Int64>(availableBytes / (1024 * 1024));
        
        if (totalBytes > 0) {
            memStatus["usage_percent"] = (static_cast<double>(usedBytes) / static_cast<double>(totalBytes)) * 100.0;
        } else {
        ...
        try {
            ...
            if (meminfoFile.is_open()) {
                ...
                if (cached > 0) {
                    memStatus["cached_mib"] = static_cast<Json::Int64>(cached / (1024 * 1024));
                }
                if (buffers > 0) {
                    memStatus["buffers_mib"] = static_cast<Json::Int64>(buffers / (1024 * 1024));
                }
            }
        } catch (...) {
...
HttpResponsePtr SystemInfoHandler::createErrorResponse(int statusCode, const std::string& error, const std::string& message) const
{
