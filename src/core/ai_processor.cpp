...
bool AIProcessor::start(const std::string& config, ResultCallback callback)
{
    ...
    processing_thread_ = std::make_unique<std::thread>(&AIProcessor::processingLoop, this);
...
void AIProcessor::processingLoop()
{
    ...
    while (!should_stop_.load() && status_.load() == Status::Running) {
        try {
            ...
            // Update metrics
            {
                ...
                if (metrics_.frames_processed > 0) {
                    metrics_.avg_latency_ms = 
                        (metrics_.avg_latency_ms * (metrics_.frames_processed - 1) + latency) / 
                        metrics_.frames_processed;
                }
                ...
                if (elapsed >= 1) {
                    metrics_.fps = static_cast<double>(frame_count_since_last_calc_) / elapsed;
                    frame_count_since_last_calc_ = 0;
            ...
            std::cerr << "[AIProcessor] Error in processing loop: " << e.what() << std::endl;
