...
void NodePoolManager::initializeDefaultTemplates() {
    ...
    // Step 2: Override with manual templates for special cases or enhanced defaults
    // These templates may have more detailed default parameters or special configurations
    // ========== SOURCE NODES ==========
    ...
    yunetDetector.optionalParameters = {"score_threshold", "nms_threshold", "top_k"};
    ...
    rtmpSrc.optionalParameters = {"channel", "resize_ratio", "skip_interval", "gst_decoder_name"};
    ...
    baCrossline.optionalParameters = {"line_channel", "line_start_x", "line_start_y", "line_end_x", "line_end_y"};
    ...
    trtYOLOv8Classifier.description = "Image classification using TensorRT YOLOv8";
    ...
    trtVehicleFeatureEncoder.description = "Extract vehicle features using TensorRT";
    ...
    trtVehicleColorClassifier.templateId = "trt_vehicle_color_classifier_template";
    ...
    trtVehicleColorClassifier.description = "Classify vehicle color using TensorRT";
    ...
    trtVehicleScanner.description = "Vehicle scanning and analysis using TensorRT";
    ...
    // Templates are created but nodes won't be usable until broker nodes are re-enabled
...
std::vector<NodePoolManager::NodeTemplate> NodePoolManager::getAllTemplates() const {
...
std::vector<NodePoolManager::NodeTemplate> NodePoolManager::getTemplatesByCategory(const std::string& category) const {
    std::shared_lock<std::shared_timed_mutex> lock(mutex_);
...
std::optional<NodePoolManager::NodeTemplate> NodePoolManager::getTemplate(const std::string& templateId) const {
    std::shared_lock<std::shared_timed_mutex> lock(mutex_);
...
std::string NodePoolManager::createPreConfiguredNode(
    ...
    // Merge parameters
    std::map<std::string, std::string> finalParams = nodeTemplate.defaultParameters;
    ...
    // node.node = createNodeInstance(nodeTemplate, finalParams);  // TODO: Implement
...
std::optional<NodePoolManager::PreConfiguredNode> NodePoolManager::getPreConfiguredNode(const std::string& nodeId) const {
    std::shared_lock<std::shared_timed_mutex> lock(mutex_);
...
std::vector<NodePoolManager::PreConfiguredNode> NodePoolManager::getAllPreConfiguredNodes() const {
    std::shared_lock<std::shared_timed_mutex> lock(mutex_);
...
std::vector<NodePoolManager::PreConfiguredNode> NodePoolManager::getAvailableNodes() const {
    std::shared_lock<std::shared_timed_mutex> lock(mutex_);
...
std::optional<SolutionConfig> NodePoolManager::buildSolutionFromNodes(
...
size_t NodePoolManager::createDefaultNodesFromTemplates() {
    ...
    // Iterate through all templates and create nodes for those that can be created
    for (const auto& [templateId, nodeTemplate] : templates_) {
        // Skip if we've already created a node for this type
        if (createdNodeTypes.find(nodeTemplate.nodeType) != createdNodeTypes.end()) {
            continue;
        }
        
        // Check if we can create a node (all required parameters must have defaults or be optional)
        bool canCreate = true;
        ...
        // Check required parameters - if any required parameter doesn't have a default, skip
        for (const auto& required : nodeTemplate.requiredParameters) {
        ...
        if (!canCreate) {
            std::cerr << "[NodePoolManager] Skipping template " << templateId 
                      ...
                      << ") - missing required parameters without defaults" << std::endl;
            continue;
        }
        ...
        std::cerr << "[NodePoolManager] Created default node: " << node.nodeId 
                  << " (type: " << nodeTemplate.nodeType << ", template: " << templateId << ")" << std::endl;
    }
    
    std::cerr << "[NodePoolManager] Created " << nodesCreated << " default nodes from templates" << std::endl;
    return nodesCreated;
}

size_t NodePoolManager::createNodesFromDefaultSolutions(SolutionRegistry& solutionRegistry) {
    std::unique_lock<std::shared_timed_mutex> lock(mutex_);
    ...
    for (const auto& [solutionId, solutionConfig] : allSolutions) {
        ...
        for (const auto& nodeConfig : solutionConfig.pipeline) {
            ...
            if (templateIt == nodeTypeToTemplateId.end()) {
                std::cerr << "[NodePoolManager] Warning: No template found for node type: " << nodeType << std::endl;
                continue;
            }
            ...
            std::cerr << "[NodePoolManager] Created pre-configured node: " << node.nodeId 
                      << " (type: " << nodeType << ", template: " << templateId << ")" << std::endl;
        }
    }
...
size_t NodePoolManager::loadNodesFromStorage(NodeStorage& nodeStorage) {
    ...
    for (const auto& node : loadedNodes) {
        // Check if template exists - direct access since we already have unique_lock
        auto templateIt = templates_.find(node.templateId);
        if (templateIt == templates_.end()) {
            std::cerr << "[NodePoolManager] Warning: Template not found for node " << node.nodeId 
                      << ": " << node.templateId << ". Skipping." << std::endl;
            skippedCount++;
            continue;
        }
    ...
    std::cerr << "[NodePoolManager] Loaded " << loadedCount << " user-created nodes from storage" << std::endl;
    if (skippedCount > 0) {
        std::cerr << "[NodePoolManager] Skipped " << skippedCount << " nodes (already exist or invalid)" << std::endl;
    }
    return loadedCount;
}
