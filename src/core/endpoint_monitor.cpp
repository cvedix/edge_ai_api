                                   ...
                                   bool is_error)
{
    ...
    if (it == endpoint_stats_.end()) {
        it = endpoint_stats_.emplace(endpoint, std::make_shared<EndpointStats>()).first;
    }
    ...
    while (response_time_ms > current_max &&
           !stats->max_response_time_ms.compare_exchange_weak(current_max, response_time_ms)) {
        current_max = stats->max_response_time_ms.load();
    }

    uint64_t current_min = stats->min_response_time_ms.load();
    while (response_time_ms < current_max &&
           !stats->min_response_time_ms.compare_exchange_weak(current_min, response_time_ms)) {
...
std::shared_ptr<EndpointMonitor::EndpointStats> EndpointMonitor::getStats(const std::string& endpoint) const
{
    std::lock_guard<std::mutex> lock(stats_mutex_);
    auto it = endpoint_stats_.find(endpoint);
    if (it != endpoint_stats_.end()) {
...
std::map<std::string, std::shared_ptr<EndpointMonitor::EndpointStats>> EndpointMonitor::getAllStats() const
{
    std::lock_guard<std::mutex> lock(stats_mutex_);
