...
CircuitBreaker::CircuitBreaker(size_t failure_threshold,
    ...
    : failure_threshold_(failure_threshold),
      success_threshold_(success_threshold), timeout_ms_(timeout_ms),
      last_failure_time_(std::chrono::steady_clock::now()),
      last_success_time_(std::chrono::steady_clock::now()) {}

CircuitBreaker::State CircuitBreaker::getState() const { return state_.load(); }
...
CircuitBreaker::Stats CircuitBreaker::getStats() const {
  ...
  stats.success_rate =
      total > 0 ? static_cast<double>(stats.success_calls) / total : 0.0;
