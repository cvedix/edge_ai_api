...
void BackpressureController::configure(const std::string& instanceId,
    ...
    config.min_frame_interval = std::chrono::milliseconds(
        static_cast<int>(1000.0 / config.max_fps));
...
void BackpressureController::recordFrameProcessed(const std::string& instanceId) {
    ...
    static thread_local std::unordered_map<std::string, std::chrono::steady_clock::time_point> last_fps_update;
    static thread_local std::unordered_map<std::string, uint64_t> frame_count_since_update;
...
double BackpressureController::getCurrentFPS(const std::string& instanceId) const {
...
double BackpressureController::getTargetFPS(const std::string& instanceId) const {
    auto it = stats_.find(instanceId);
    if (it != stats_.end()) {
...
bool BackpressureController::isBackpressureDetected(const std::string& instanceId) const {
    auto it = stats_.find(instanceId);
    if (it != stats_.end()) {
...
BackpressureController::BackpressureStatsSnapshot BackpressureController::getStats(const std::string& instanceId) const {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = stats_.find(instanceId);
    if (it != stats_.end()) {
        const BackpressureStats& stats = it->second;
        BackpressureStatsSnapshot snapshot;
        snapshot.frames_dropped = stats.frames_dropped.load(std::memory_order_relaxed);
        snapshot.frames_processed = stats.frames_processed.load(std::memory_order_relaxed);
        snapshot.queue_full_count = stats.queue_full_count.load(std::memory_order_relaxed);
        ...
        snapshot.backpressure_detected = stats.backpressure_detected.load(std::memory_order_relaxed);
...
void BackpressureController::updateAdaptiveFPS(const std::string& instanceId) {
    ...
    static thread_local std::unordered_map<std::string, std::chrono::steady_clock::time_point> last_adaptive_update;
    auto now = std::chrono::steady_clock::now();
    ...
    bool backpressure = stats.backpressure_detected.load(std::memory_order_relaxed);
    ...
    if (backpressure || queue_full > 0) {
        ...
        config.min_frame_interval = std::chrono::milliseconds(
            static_cast<int>(1000.0 / new_target));
