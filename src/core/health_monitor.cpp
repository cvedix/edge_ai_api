...
void HealthMonitor::start(Watchdog& watchdog)
{
    ...
    watchdog_ = &watchdog;
    running_.store(true);
    monitor_thread_ = std::make_unique<std::thread>(&HealthMonitor::monitorLoop, this);

    std::cout << "[HealthMonitor] Started (interval=" << monitor_interval_ms_ << "ms)" << std::endl;
}
...
HealthMonitor::HealthMetrics HealthMonitor::collectMetrics()
{
    ...
    metrics.cpu_usage_percent = 0.0; // TODO: Implement proper CPU usage calculation
