...
void Watchdog::start(RecoveryCallback recovery_callback)
{
    ...
    watchdog_thread_ = std::make_unique<std::thread>(&Watchdog::watchdogLoop, this);
...
void Watchdog::watchdogLoop()
{
    ...
    while (running_.load()) {
        ...
        // Check health
        if (!checkHealth()) {
            std::cerr << "[Watchdog] WARNING: Application appears to be unresponsive!" << std::endl;
            ...
            if (recovery_callback_) {
                ...
                try {
                    ...
                    std::cerr << "[Watchdog] Recovery action failed: " << e.what() << std::endl;
                }
            } else {
                std::cerr << "[Watchdog] No recovery callback set. Application may be dead." << std::endl;
            }
        } else {
            {
                std::lock_guard<std::mutex> stats_lock(stats_mutex_);
