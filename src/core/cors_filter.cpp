                         ...
                         drogon::FilterChainCallback&& fccb)
{
    ...
    if (req->method() == drogon::Options) {
        ...
        std::cerr << "[CorsFilter] OPTIONS response sent with ALLOW ALL headers" << std::endl;
        fcb(resp);
        return;
    }
    
    // For all other requests, continue to handler and add CORS headers to response
    // Wrap the callback to add CORS headers
    auto wrapped_fcb = [fcb = std::move(fcb)](const drogon::HttpResponsePtr& resp) {
    ...
    // Note: We need to wrap the callback, but Drogon's filter chain doesn't support this directly
    // So we'll let handlers add CORS headers themselves, and this filter handles OPTIONS
    fccb();
}
