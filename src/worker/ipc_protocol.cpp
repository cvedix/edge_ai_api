...
bool MessageHeader::deserialize(const char *data, size_t len,
                                MessageHeader &out) {
  if (len < HEADER_SIZE) {
    return false;
  }
  ...
  for (int i = 7; i >= 0; --i) {
    out.payload_size =
        (out.payload_size << 8) | static_cast<uint8_t>(data[8 + i]);
  }
...
bool IPCMessage::deserialize(const std::string &data, IPCMessage &out) {
  ...
  if (header.payload_size > 0) {
    std::string payload_str(data.data() + MessageHeader::HEADER_SIZE,
                            header.payload_size);
