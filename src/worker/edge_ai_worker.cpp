...
int main(int argc, char* argv[]) {
    ...
    if (!args.valid) {
        ...
        std::cerr << "[Worker] Usage: edge_ai_worker --instance-id <id> --socket <path> [--config <json>]" << std::endl;
        return 1;
    }
    ...
    worker::WorkerHandler handler(args.instance_id, args.socket_path, args.config);
    g_handler = &handler;
