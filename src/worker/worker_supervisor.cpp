...
bool WorkerSupervisor::spawnWorker(const std::string& instance_id, const Json::Value& config) {
    std::lock_guard<std::mutex> lock(workers_mutex_);
    
    // Check if worker already exists
    if (workers_.find(instance_id) != workers_.end()) {
        std::cerr << "[Supervisor] Worker already exists for instance: " << instance_id << std::endl;
        return false;
    }
    ...
    if (exe_path.empty()) {
        std::cerr << "[Supervisor] Worker executable not found: " << worker_executable_ << std::endl;
        return false;
    }
    ...
    if (pid == 0) {
        ...
        // Arguments: worker_executable --instance-id <id> --socket <path> --config <json>
        execl(exe_path.c_str(), 
              exe_path.c_str(),
              "--instance-id", instance_id.c_str(),
    ...
    std::cout << "[Supervisor] Spawned worker PID " << pid << " for instance: " << instance_id << std::endl;
    
    // Wait for worker to become ready
    bool ready = waitForWorkerReady(*worker, worker_startup_timeout_ms_);
    
    if (!ready) {
        std::cerr << "[Supervisor] Worker failed to become ready, killing PID " << pid << std::endl;
        kill(pid, SIGKILL);
...
bool WorkerSupervisor::terminateWorker(const std::string& instance_id, bool force) {
    std::lock_guard<std::mutex> lock(workers_mutex_);
    ...
    if (!force && worker.client && worker.client->isConnected()) {
        ...
        for (int i = 0; i < 10; ++i) {
            ...
            if (result > 0) {
                std::cout << "[Supervisor] Worker " << instance_id << " exited gracefully" << std::endl;
    ...
    if (kill(worker.pid, sig) == 0) {
        ...
        std::cout << "[Supervisor] Worker " << instance_id << " terminated with signal " << sig << std::endl;
    }
...
IPCMessage WorkerSupervisor::sendToWorker(const std::string& instance_id, 
    ...
    if (it == workers_.end()) {
        ...
        error.payload = createErrorResponse("Worker not found", ResponseStatus::NOT_FOUND);
        return error;
    }
    ...
    if (!worker.client || !worker.client->isConnected()) {
        IPCMessage error;
        error.type = MessageType::ERROR_RESPONSE;
        error.payload = createErrorResponse("Worker not connected", ResponseStatus::ERROR);
        return error;
    }
    ...
    if (worker.state != WorkerState::READY && worker.state != WorkerState::BUSY) {
        IPCMessage error;
        error.type = MessageType::ERROR_RESPONSE;
        error.payload = createErrorResponse("Worker not ready", ResponseStatus::ERROR);
        return error;
    }
...
WorkerState WorkerSupervisor::getWorkerState(const std::string& instance_id) const {
    std::lock_guard<std::mutex> lock(workers_mutex_);
...
std::optional<WorkerInfo> WorkerSupervisor::getWorkerInfo(const std::string& instance_id) const {
    std::lock_guard<std::mutex> lock(workers_mutex_);
...
void WorkerSupervisor::handleWorkerCrash(const std::string& instance_id) {
    ...
    if (worker.restart_count < max_restarts_) {
        std::cout << "[Supervisor] Attempting restart " << (worker.restart_count + 1) 
                  << "/" << max_restarts_ << " for " << instance_id << std::endl;
        ...
        std::cerr << "[Supervisor] Max restarts reached for " << instance_id << std::endl;
        cleanupWorker(worker);
        workers_.erase(it);
    }
}
...
bool WorkerSupervisor::waitForWorkerReady(WorkerInfo& worker, int timeout_ms) {
    ...
    while (true) {
        ...
        if (!worker.client->isConnected()) {
            if (worker.client->connect(1000)) {
                ...
                if (ready_msg.type == MessageType::WORKER_READY) {
                    setWorkerState(worker, WorkerState::READY);
                    worker.last_heartbeat = std::chrono::steady_clock::now();
                    std::cout << "[Supervisor] Worker " << worker.instance_id << " is ready" << std::endl;
                    return true;
                }
            }
        }
        ...
        retry_delay = std::min(retry_delay * 2, 1000);  // Exponential backoff, max 1s
    }
}

void WorkerSupervisor::setWorkerState(WorkerInfo& worker, WorkerState new_state) {
...
std::string WorkerSupervisor::findWorkerExecutable() const {
    ...
    if (path_env) {
        ...
        while ((end = path_str.find(':', start)) != std::string::npos) {
            ...
            std::filesystem::path worker_path = std::filesystem::path(dir) / worker_executable_;
            if (std::filesystem::exists(worker_path)) {
                return worker_path.string();
            }
            start = end + 1;
