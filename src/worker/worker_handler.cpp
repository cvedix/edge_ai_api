...
bool WorkerHandler::initializeDependencies() {
    std::cout << "[Worker:" << instance_id_ << "] Initializing dependencies..." << std::endl;
    ...
    try {
        ...
        std::cout << "[Worker:" << instance_id_ << "] Dependencies initialized" << std::endl;
        return true;
    } catch (const std::exception& e) {
        std::cerr << "[Worker:" << instance_id_ << "] Failed to initialize dependencies: " 
                  << e.what() << std::endl;
        last_error_ = e.what();
        return false;
    }
...
int WorkerHandler::run() {
    ...
    if (!initializeDependencies()) {
        std::cerr << "[Worker:" << instance_id_ << "] Failed to initialize dependencies" << std::endl;
        return 1;
    }
    ...
    if (!server_->start([this](const IPCMessage& msg) {
        return handleMessage(msg);
    })) {
        std::cerr << "[Worker:" << instance_id_ << "] Failed to start IPC server" << std::endl;
        return 1;
    }
    ...
    if (!config_.isNull() && config_.isMember("Solution")) {
        if (!buildPipeline()) {
            std::cerr << "[Worker:" << instance_id_ << "] Failed to build initial pipeline" << std::endl;
    ...
    std::cout << "[Worker:" << instance_id_ << "] Ready and listening on " << socket_path_ << std::endl;
...
void WorkerHandler::requestShutdown() {
...
IPCMessage WorkerHandler::handleMessage(const IPCMessage& msg) {
    switch (msg.type) {
        ...
        default: {
            ...
            error.payload = createErrorResponse("Unknown message type", ResponseStatus::INVALID_REQUEST);
            return error;
        }
    }
}
...
IPCMessage WorkerHandler::handleCreateInstance(const IPCMessage& msg) {
    ...
    if (!pipeline_nodes_.empty()) {
        response.payload = createErrorResponse("Instance already exists", ResponseStatus::ALREADY_EXISTS);
        return response;
    }
    ...
    if (!buildPipeline()) {
        response.payload = createErrorResponse("Failed to build pipeline: " + last_error_, 
...
IPCMessage WorkerHandler::handleStartInstance(const IPCMessage& /*msg*/) {
    ...
    if (pipeline_nodes_.empty()) {
        response.payload = createErrorResponse("No pipeline configured", ResponseStatus::NOT_FOUND);
        return response;
    }
    
    if (pipeline_running_) {
        response.payload = createErrorResponse("Pipeline already running", ResponseStatus::ALREADY_EXISTS);
        return response;
    }
    
    if (!startPipeline()) {
        response.payload = createErrorResponse("Failed to start pipeline: " + last_error_,
                                               ResponseStatus::INTERNAL_ERROR);
        return response;
    }
...
IPCMessage WorkerHandler::handleGetStatistics(const IPCMessage& /*msg*/) {
    ...
    Json::Value data;
    data["instance_id"] = instance_id_;
    data["frames_processed"] = static_cast<Json::UInt64>(frames_processed_.load());
    data["dropped_frames_count"] = static_cast<Json::UInt64>(dropped_frames_.load());
...
bool WorkerHandler::buildPipeline() {
    std::cout << "[Worker:" << instance_id_ << "] Building pipeline from config..." << std::endl;
    ...
    try {
        ...
        auto optSolution = SolutionRegistry::getInstance().getSolution(req.solution);
        ...
        // Build pipeline
        pipeline_nodes_ = pipeline_builder_->buildPipeline(optSolution.value(), req, instance_id_);
        ...
        last_error_ = e.what();
        std::cerr << "[Worker:" << instance_id_ << "] Failed to build pipeline: " 
                  << e.what() << std::endl;
        return false;
    }
}
...
bool WorkerHandler::startPipeline() {
    ...
    std::cout << "[Worker:" << instance_id_ << "] Starting pipeline..." << std::endl;
    
    try {
        // Find and start source node (first node in pipeline)
        // Try RTSP source first
        auto rtspNode = std::dynamic_pointer_cast<cvedix_nodes::cvedix_rtsp_src_node>(
            pipeline_nodes_[0]);
        if (rtspNode) {
            std::cout << "[Worker:" << instance_id_ << "] Starting RTSP source node" << std::endl;
            rtspNode->start();
        } else {
            // Try file source
            auto fileNode = std::dynamic_pointer_cast<cvedix_nodes::cvedix_file_src_node>(
                pipeline_nodes_[0]);
            if (fileNode) {
                std::cout << "[Worker:" << instance_id_ << "] Starting file source node" << std::endl;
                fileNode->start();
            } else {
        ...
        std::cout << "[Worker:" << instance_id_ << "] Pipeline started" << std::endl;
        return true;
        
    } catch (const std::exception& e) {
        last_error_ = e.what();
        std::cerr << "[Worker:" << instance_id_ << "] Failed to start pipeline: " 
                  << e.what() << std::endl;
        return false;
    }
}
...
void WorkerHandler::stopPipeline() {
    ...
    std::cout << "[Worker:" << instance_id_ << "] Stopping pipeline..." << std::endl;
    ...
    try {
        ...
        std::cerr << "[Worker:" << instance_id_ << "] Error stopping pipeline: " 
                  << e.what() << std::endl;
    }
...
void WorkerHandler::sendReadySignal() {
    std::cout << "[Worker:" << instance_id_ << "] Sending ready signal" << std::endl;
}
...
std::string WorkerHandler::encodeFrameToBase64(const cv::Mat& frame, int quality) const {
    if (frame.empty()) {
        return "";
    }
...
WorkerArgs WorkerArgs::parse(int argc, char* argv[]) {
    ...
    while ((opt = getopt_long(argc, argv, "i:s:c:h", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'i':
            ...
            case 'h':
                args.error = "Usage: edge_ai_worker --instance-id <id> --socket <path> [--config <json>]";
                return args;
            default:
