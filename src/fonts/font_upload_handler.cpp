...
std::string FontUploadHandler::getFontsDirectory() const {
    return fonts_dir_;
}
...
std::string FontUploadHandler::sanitizeFilename(const std::string& filename) const {
...
std::string FontUploadHandler::extractFontName(const HttpRequestPtr &req) const {
    ...
    try {
        ...
        if (isMultipart) {
            ...
            try {
                ...
                callback(createErrorResponse(500, "Directory creation failed", 
                    "Could not create fonts directory: " + std::string(e.what())));
                return;
            }
            ...
            while (partStart != std::string::npos) {
                ...
                try {
                    ...
                    fileCount++;
                    std::cerr << "[FontUploadHandler] Font file uploaded: " << finalFilename 
            ...
            if (fileCount > 0) {
                ...
                response["message"] = "Uploaded " + std::to_string(fileCount) + " font file(s)";
            ...
            if (!isValidFontFile(originalFilename)) {
                callback(createErrorResponse(400, "Invalid file type", 
                    "Only font files (.ttf, .otf, .woff, .woff2, .eot, .ttc, etc.) are allowed"));
                return;
            }
            ...
            if (std::filesystem::exists(filePath)) {
                ...
                std::string extension = basePath.extension().string();  // .ttf, .otf, etc.
                ...
                std::cerr << "[FontUploadHandler] File with original name exists, using: " << sanitizedFilename << std::endl;
            }
        ...
        try {
            ...
            try {
                ...
                std::cerr << "[FontUploadHandler] Error getting file size: " << e.what() << std::endl;
            ...
            try {
                ...
                std::cerr << "[FontUploadHandler] Error getting canonical path: " << e.what() << std::endl;
                // Use original path as fallback
                canonicalPath = filePath;
            }
            
        } catch (const std::exception& e) {
            std::cerr << "[FontUploadHandler] Unexpected error: " << e.what() << std::endl;
...
void FontUploadHandler::listFonts(
    ...
    try {
        ...
        for (const auto& entry : std::filesystem::directory_iterator(fontsPath)) {
            if (entry.is_regular_file() && isValidFontFile(entry.path().filename().string())) {
                Json::Value font;
                font["filename"] = entry.path().filename().string();
                font["path"] = std::filesystem::canonical(entry.path()).string();
                font["size"] = static_cast<Json::Int64>(std::filesystem::file_size(entry.path()));
...
void FontUploadHandler::renameFont(
    ...
    try {
        ...
        if (fontName.empty()) {
            callback(createErrorResponse(400, "Invalid request", "Font name is required"));
            return;
        }
        ...
        if (sanitizedSourceName.empty() || sanitizedSourceName != fontName) {
            callback(createErrorResponse(400, "Invalid filename", "Invalid source font name"));
            return;
        }
        ...
        if (!isValidFontFile(newName)) {
            callback(createErrorResponse(400, "Invalid file type", 
                "New name must have a valid font file extension (.ttf, .otf, .woff, .woff2, etc.)"));
            return;
        }
        ...
        // Build file paths
        std::string fontsDir = getFontsDirectory();
        std::filesystem::path sourcePath = std::filesystem::path(fontsDir) / sanitizedSourceName;
        std::filesystem::path destPath = std::filesystem::path(fontsDir) / sanitizedNewName;
        
        // Check if source file exists
        if (!std::filesystem::exists(sourcePath)) {
            callback(createErrorResponse(404, "Not found", "Source font file not found"));
            return;
        }
        ...
        if (std::filesystem::exists(destPath)) {
            callback(createErrorResponse(409, "File exists", 
                "A font file with the new name already exists. Please delete it first or use a different name."));
            return;
        }
        ...
        try {
            ...
            callback(createErrorResponse(500, "Rename failed", 
                "Could not rename font file: " + std::string(e.what())));
            return;
        }
        ...
        std::cerr << "[FontUploadHandler] Font file renamed: " << sanitizedSourceName 
...
void FontUploadHandler::deleteFont(
    ...
    try {
        ...
        if (sanitizedFilename.empty() || sanitizedFilename != fontName) {
            callback(createErrorResponse(400, "Invalid filename", "Invalid font name"));
            return;
        }
        
        // Build file path
        std::string fontsDir = getFontsDirectory();
        std::filesystem::path filePath = std::filesystem::path(fontsDir) / sanitizedFilename;
        ...
        if (!std::filesystem::remove(filePath)) {
            callback(createErrorResponse(500, "Delete failed", "Could not delete font file"));
            return;
        }
        ...
        std::cerr << "[FontUploadHandler] Font file deleted: " << sanitizedFilename << std::endl;
...
void FontUploadHandler::handleOptions(
    ...
    std::cerr << "[FontUploadHandler] Origin: " << req->getHeader("Origin") << std::endl;
    
    // Use CORS helper to create "allow all" response
    auto resp = CorsHelper::createOptionsResponse();
    
    std::cerr << "[FontUploadHandler] OPTIONS response sent with ALLOW ALL headers" << std::endl;
    
    callback(resp);
}

HttpResponsePtr FontUploadHandler::createErrorResponse(
    int statusCode,
