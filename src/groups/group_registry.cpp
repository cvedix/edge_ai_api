...
bool GroupRegistry::registerGroup(const std::string& groupId, 
    ...
    if (groups_.find(groupId) != groups_.end()) {
        std::cerr << "[GroupRegistry] Group " << groupId << " already exists" << std::endl;
        return false;
    }
    ...
    std::cerr << "[GroupRegistry] Registered group: " << groupId << " (" << group.groupName << ")" << std::endl;
    return true;
}

std::optional<GroupInfo> GroupRegistry::getGroup(const std::string& groupId) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
bool GroupRegistry::updateGroup(const std::string& groupId,
    ...
    auto it = groups_.find(groupId);
    if (it == groups_.end()) {
        std::cerr << "[GroupRegistry] Group " << groupId << " not found" << std::endl;
        return false;
    }
    ...
    // Check if read-only
    if (group.readOnly) {
        std::cerr << "[GroupRegistry] Cannot update read-only group " << groupId << std::endl;
        return false;
    }
...
bool GroupRegistry::deleteGroup(const std::string& groupId) {
    ...
    // Check if default
    if (group.isDefault) {
        std::cerr << "[GroupRegistry] Cannot delete default group " << groupId << std::endl;
        return false;
    }
    ...
    if (instancesIt != group_instances_.end() && !instancesIt->second.empty()) {
        std::cerr << "[GroupRegistry] Cannot delete group " << groupId << " with instances" << std::endl;
        return false;
    }
...
void GroupRegistry::updateInstanceCountUnlocked(const std::string& groupId, int count) {
...
std::vector<std::string> GroupRegistry::getInstanceIds(const std::string& groupId) const {
    std::lock_guard<std::mutex> lock(mutex_);
...
void GroupRegistry::setInstanceIds(const std::string& groupId, const std::vector<std::string>& instanceIds) {
    std::lock_guard<std::mutex> lock(mutex_);
